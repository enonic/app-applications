...
export class ListMarketApplicationsRequest {
    ...
    private xpVersionPattern: string;
    private xpVersionFull: string;
    private url: string = 'https://market.enonic.com/api/graphql';
    ...
    setXpVersion(xpVersion: string, preprocess: boolean = true): ListMarketApplicationsRequest {
        if (preprocess) {
            const cleanVersion = xpVersion.replace(/-.*$/, '');
            this.xpVersionFull = cleanVersion;
            this.xpVersionPattern = cleanVersion.split('.')[0] + '.*';
        } else {
            this.xpVersionFull = xpVersion;
            this.xpVersionPattern = xpVersion;
        }
        return this;
    }
    ...
    private matchesXpVersion(supportedVersion: string): boolean {
        ...
        //   "7.*"    -> true  (wildcard matches major version)
        //   "7.15.*" -> true  (wildcard after matching prefix)

        const xpParts = this.xpVersionFull.split('.').map(Number);
        ...
        for (let i = 0; i < supportedParts.length; i++) {
            ...
            if (supportedPart === '*') {
                // Wildcard - check major version matches and we're done
                return true;
            }
